                  ...
                  <ul className="list-group">
                    {timeslots
                      ...
                      .map((timeslot) => (
                        <li
                          key={timeslot.id}
                          className={`list-group-item ${selectedTimeslot.some((slot) => slot.id === timeslot.id) ? 'active' : ''} ${
                            isSlotOccupied(timeslot) ? 'list-group-item-dark' : ''
                          }`}
                          onClick={() => this.handleSelectTimeslot(timeslot)}
                        >
                          {timeslot.start_time} - {timeslot.end_time} {isSlotOccupied(timeslot) ? 'Занято' : 'Свободно'}
                        </li>
                      ))}
                  </ul>
                </div>
